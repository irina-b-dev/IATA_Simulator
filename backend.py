...
def apply_operations(target_list, starting_qubit, control_qubits = [], gate_name = "", gate_matrix = [1], noise = False, name = -1):
    if len(control_qubits) > 0:
        first_qubit = np.min([starting_qubit,np.min(control_qubits)])
        control_gate_name = f"Controlled-{gate_name}_Cq{control_qubits}_Tq{starting_qubit}"
        print(control_gate_name)
        gate = target_list.control_gate(control_qubits = control_qubits, target_qubit = starting_qubit, gate_matrix = gates_map[gate_name][0], name=control_gate_name)
        target_list.apply_gate(gate, starting_qubit = first_qubit)
        pass
    else:
        match gate_name:
            case "H":
